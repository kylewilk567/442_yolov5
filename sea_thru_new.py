...
def run(args):
    ...
    sigma_est = np.mean([estimate_sigma(recovered[:, :, i]) for i in range(recovered.shape[-1])]) / 10.0

    #recovered = denoise_tv_chambolle(recovered, sigma_est, multichannel=True)
    recovered = np.stack([denoise_tv_chambolle(recovered[:, :, i], sigma_est) for i in range(recovered.shape[-1])],
                         axis=-1)
 ...
 #python3 sea_thru_new.py --image test_img6.jpg --output output6.jpg --f 2.0 --l 0.1
