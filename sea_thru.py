...
def get_raw_img(filename):
    with Raw(filename=filename) as raw:
        buf = np.array(raw.to_buffer())
    return np.reshape(buf, (raw.metadata.height, raw.metadata.width, -1))
...
def refine_wideband_attentuation(depths, illum, estimation, restarts=10, min_depth_fraction=0.1,
                                 max_mean_loss_fraction=np.inf, l=1.0, radius_fraction=0.01):
    eps = 1E-8
    z_max, z_min = np.max(depths), np.min(depths)
...
def run_pipeline(img, depths, args):
    ...
    refined_beta_D_r, coefsR = refine_wideband_attentuation(depths, illR, beta_D_r,
                                                            radius_fraction=args.spread_data_fraction, l=args.l)
    ...
    if args.output_graphs:
        ...
        locs = np.where(
            np.logical_and(beta_D_r > eps,
                           np.logical_and(beta_D_g > eps, np.logical_and(depths > eps, beta_D_b > eps))))
...
if __name__ == '__main__':
    ...
    if args.preprocess_for_monodepth:
        ...
        im, depths = load_image_and_depth_map(args.image, args.depth_map, args.size)
        if args.monodepth:
        ...
        recovered = run_pipeline(im, depths, args)
